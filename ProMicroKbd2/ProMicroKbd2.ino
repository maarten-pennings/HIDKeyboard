...
// 2-key keyboard: PageUp/PageDown
// Maarten Pennings 2019 mar 30
...
// It has 2 buttons, both represented by a box. The top line in the box
// shows the button index (BUT0, BUT1). 
//
//                                      +---------+
//                                     1|TX    RAW| 
//                                     0|RX    GND|
//   +------+------+------+             |GND   RST|
//   | BUT0 |      | BUT1 |             |GND   VCC|
//   | PIN15|      | PIN7 |            2|SDA    A3|21
//   |PageUp|      |PageDn|            3|SCL    A2|20
//   +------+------+------+            4|A6     A1|19
//                                     5|       A0|18
//                                     6|A7   SCLK|15
//                                     7|     MISO|14
...
//                                      +---------+
//
// Each button is a capacitive switch: it has 3V3 and GND, and the signal
// pin is connected to an arduino pin. A button press raises the signal
// pin to 3V3, a "high".
//
...
// sketch assigns to each button in the middle row (PIN15, PIN7). In case you 
// wonder why these pins: look at the photo: a capacitive switch board is
// soldered to either side.
//
...
void but_init(void) {
  for( int i=0; i<BUT_COUNT; i++ )
    pinMode(BUT_PINS[i],INPUT); // Configure pins as input, without pull-up
  but_scan();
  but_scan();
}
...
void but_scan(void) {
  ...
  but_state_cur= 0;
  for( int i=0; i<BUT_COUNT; i++ )
    if( digitalRead(BUT_PINS[i])==1 ) but_state_cur |= 1<<i; // butt on press make pin 1
}
